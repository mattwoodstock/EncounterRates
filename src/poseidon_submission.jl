    ...
    for velo2 in velos2
        for prey_vis in prey_visual
            for (i, n) in enumerate(n_pz)
                        ...
                        file_path = "Prey $preys _ Speed $speed _ Vis $prey_vis.jld"
                        jldopen(file_path, "w") do file
                            # Write each DataFrame to the file with a unique key
                            file["all_results"] = res.all_res
                            file["Densities"] = Tables.table(res.densities)
                            file["Encounters"] = Tables.table(res.encounters)
                            file["Consumption"] = Tables.table(res.consumed)
                            file["Prey_Prey_distance_init"] = Tables.table(res.prey_prey_init)
                            file["Prey_Prey_distance_final"] = Tables.table(res.prey_prey_final)
                            file["Pred_Prey_distance_init"] = Tables.table(res.pred_prey_init)
                            file["Pred_Prey_distance_final"] = Tables.table(res.pred_prey_final)
                            file["Pred_Pred_distance_init"] = Tables.table(res.pred_pred_init)
                            file["Pred_Pred_distance_final"] = Tables.table(res.pred_pred_final)
