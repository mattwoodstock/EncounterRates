...
function plot_model(res::Results,model::PlanktonModel, z_pause::Vector{Float64}, ΔT::Float64, deactivate::Bool, encounter_diags::Dict{String,Any},densities::Matrix{Int},isubexp::Int64,iteration::Int64, locations::Array{Float64,4}; z_pause_ΔT=0.0::Float64, velo1=1e-4::Float64, velo2 = 1e-5::Float64, encounter_tresh=0.001::Float64,prey_vis=1::Float64, pred_vis = 1::Float64)
    ...
    if velo2 > 0.0
        ...
        for (ip, iz_model) in enumerate(ip_ac_int)
            ...
            if dist_3d[ip, iz] > prey_visual_range #Animal moves to random vector because closest animal is outside of visual range
                ...
                p_pos[1,ip] = mod(p_pos[1,ip],1)
                p_pos[2,ip] = mod(p_pos[2,ip],1)
                p_pos[3,ip] = mod(p_pos[3,ip],1)
                ...
                pred_positions = Matrix{Float64}(undef,1,3)

                sort_vector =sort(dist_3d[ip,:])

                if size(sub_dist,1) > 0
                    for i in 1:1 #Only use the closest for this run
    ...
    if velo1 > 0.0
        ...
        for (iz, iz_model) in enumerate(iz_ac_int)
            ...
            elseif dist_3d[ip,iz] ≤ pred_visual_range #Predator cannot catch preys, but it sees them. Moves towards closest prey
                ...
                z_pos[1,iz] = mod(z_pos[1,iz],1)
                z_pos[2,iz] = mod(z_pos[2,iz],1)
                z_pos[3,iz] = mod(z_pos[3,iz],1)
    ...
    for i in 1:n_prey
        locations[i,1,iteration,isubexp] = model.individuals.phytos.sp1.data.x[i]
        locations[i,2,iteration,isubexp] = model.individuals.phytos.sp1.data.y[i]
        locations[i,3,iteration,isubexp] = model.individuals.phytos.sp1.data.z[i]
    end
    for i in 1:n_pred
        locations[n_prey+i,1,iteration,isubexp] = model.individuals.phytos.sp2.data.x[i]
        locations[n_prey+i,2,iteration,isubexp] = model.individuals.phytos.sp2.data.y[i]
        locations[n_prey+i,3,iteration,isubexp] = model.individuals.phytos.sp2.data.z[i]
    end

	return densities, locations
end

function run_test(res::Results,N_replicates=10, N_individual=[2^8, 2^3]::Vector{Int64},densities = zeros()::Matrix{Int}, locations = zeros()::Array{Float64,4}; z_pause_ΔT=0.0::Float64, velo1=1e-4::Float64,velo2=1e-5::Float64, encounter_tresh=0.001::Float64, deactivate=false::Bool, arch=CPU(),prey_vis=1.0::Float64,pred_vis = 1.0::Float64,isubexp = 1::Int64,N_iter = 300::Int64)
    ...
    ## Toggle this to add/remove diffusion. Removing diffusion has a 0 parameter. At this magnitude, 1e-8 is a suitable velocity that doesn't overpower active movement
    ...
    for i = 1:N_replicates
        ...
        pow = PlanktonOutputWriter(save_diags=false,
                                   save_plankton=false)
    ...
    return replicate_diags["sum_encounters"], N_iter, densities, locations
end
...
function run_experiments()
    ...
    n_rep = 1 #Number of replicates to run
    ...
    ninds = maximum(n_pz)+n_z
    nsimulation = length(n_pz)*length(velos2)*length(prey_visual)
    locations = Array{Float64,4}(undef,ninds,3,N_iter,nsimulation)
    ...
    for velo2 in velos2
        for prey_vis in prey_visual
            for (i, n) in enumerate(n_pz)
                        start = now()
                        n_p = n
                        tmp, n_it, densities, locations = run_test(res,n_rep, [n_p,n_z],densities,locations; velo1=velo1, velo2 = velo2, z_pause_ΔT=z_pause_ΔT, encounter_tresh=encounter_tresh,prey_vis = prey_vis,pred_vis = pred_vis,isubexp,N_iter)
                        ...
                        CSV.write("Prey visual range results.csv",res.all_res)
                        CSV.write("Prey Densities.csv",Tables.table(res.densities), writeheader=false)
                        CSV.write("Encounters.csv",Tables.table(res.encounters), writeheader=false)
                        CSV.write("Predator Consumption.csv",Tables.table(res.consumed), writeheader=false)
                        CSV.write("Prey to Prey distance_init.csv",Tables.table(res.prey_prey_init), writeheader=false)
                        CSV.write("Prey to Prey distance_final.csv",Tables.table(res.prey_prey_final), writeheader=false)
                        CSV.write("Pred to Prey distance_init.csv",Tables.table(res.pred_prey_init), writeheader=false)
                        CSV.write("Pred to Prey distance_final.csv",Tables.table(res.pred_prey_final), writeheader=false)
                        CSV.write("Pred to Pred distance_init.csv",Tables.table(res.pred_pred_init), writeheader=false)
                        CSV.write("Pred to Pred distance_final.csv",Tables.table(res.pred_pred_final), writeheader=false)

                        file_path = "Full Results.jld"
